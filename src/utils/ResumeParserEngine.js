...
`.trim();
      ...
      if (response.ok) {
        ...
        if (candidateText) {
          ...
          return {
            fullName: aiParsed.fullName || 'Jason Miller',
            firstName: aiParsed.firstName || (aiParsed.fullName ? aiParsed.fullName.split(' ')[0] : 'Jason'),
            lastName: aiParsed.lastName || (aiParsed.fullName ? aiParsed.fullName.split(' ').slice(1).join(' ') : 'Miller'),
            email: aiParsed.email || 'email@email.com',
            phone: aiParsed.phone || '3868683442',
            location: aiParsed.location || 'Los Angeles, CA',
            linkedinUrl: aiParsed.linkedinUrl || '',
            portfolioUrl: aiParsed.portfolioUrl || '',
            targetRole: aiParsed.targetRole || 'Amazon Associate',
            experienceYears: aiParsed.experienceYears || 5,
            experienceLevel: aiParsed.experienceLevel || '5+ years',
            skills: Array.isArray(aiParsed.skills) && aiParsed.skills.length > 0 ? aiParsed.skills : ['Warehouse Operations', 'Inventory Management', 'Sanitation'],
  ...
  return {
    ...
    skills: Array.from(foundSkills).length > 0 ? Array.from(foundSkills) : ['Warehouse Operations', 'Inventory Management', 'Sanitation'],
    workExperiences: [],
    education: []
  };
}
